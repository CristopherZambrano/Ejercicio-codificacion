...
public class PedidoDao {
    ...
    public boolean insertarPedido(Pedido pd) {
        sql = String.format("insert into encabezado_pedido (fecha_venta, estado, usuario_id_usuario, total, descuento) values (now(),'%s',%s, %s, %s)", pd.getEstado(), pd.getId_usuario(), pd.getTotal(), pd.getDescuento());
    ...
    public boolean cancelarPedido(Pedido pd) {
        sql = String.format("delete from encabezado_pedido where id_encapedido = %s", pd.getId_pedido());
        return conex.modifyBD(sql);
    }
